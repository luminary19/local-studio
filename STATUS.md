...
- [x] Inspect `/usage` controller observability fallback behavior.
- [x] Add controller integration coverage for `/usage` fallback observability.
- [x] Validate `/usage` fallback observability slice.
- [x] Commit this slice.
...
- [ ] Surface observability data in `/usage` and validate it end to end. Initial route observability is surfaced, raw persistence is integration-tested, `/usage` status, latency, recent-activity, error aggregation, and inference-aggregate fallback behavior are integration-tested, and the frontend normalization boundary preserves controller observability; frontend visual rendering and full API-route coverage remain.
